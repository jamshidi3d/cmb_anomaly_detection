...
class RunInputs:
    def __init__(self, **kwargs) -> None:
        self.observable:'str'           = kwargs.get(const.KEY_OBSERVABLE,       const.OBS_T)
        self.max_sim_num:'int'          = kwargs.get(const.MAX_SIM_NUM,          1000)
        self.sims_path:'str'            = kwargs.get(const.KEY_SIMS_PATH,        None)
        self.noise_path:'str'           = kwargs.get(const.KEY_NOISE_PATH,       None)
        self.cmb_fpath:'str'            = kwargs.get(const.KEY_CMB_FPATH,        None)
        self.mask_fpath:'str'           = kwargs.get(const.KEY_MASK_FPATH,       None)
        self.nside:'int'                = kwargs.get(const.KEY_NSIDE,            64)
        ...
        self.min_pix_ratio:'float'      = kwargs.get(const.KEY_MIN_PIX_RATIO,    1)
        self.max_valid_ang:'float'      = kwargs.get(const.KEY_MAX_VALID_ANG,    0)
        self.corr_full_int:'float'      = kwargs.get(const.KEY_CORR_FULL_INT,    1)
        '''Integral of the 2pcf of the full sky'''
        self.std_full:'float'           = kwargs.get(const.KEY_STD_FULL,         1)
        '''Standard deviation of the full sky'''
        self.measure_flag:'str'         = kwargs.get(const.KEY_MEASURE_FLAG,     const.STD_FLAG)
        self.geom_flag:'str'            = kwargs.get(const.KEY_GEOM_FLAG,        const.CAP_FLAG)
        self._measure_start:'float'     = kwargs.get(const.KEY_MEASURE_START,    0)
        self._measure_stop:'float'      = kwargs.get(const.KEY_MEASURE_STOP,     180)
        self._dmeasure_samples          = kwargs.get(const.KEY_DMEASURE_SAMPLES, 1)
        '''dx of measure x'''
        self.cutoff_ratio:'float'       = kwargs.get(const.KEY_CUTOFF_RATIO,     2/3)
        self.ndata_chunks:'int'         = kwargs.get(const.KEY_NDATA_CHUNKS,     4)
        '''Number of chuncks that the data is divided to. Helpful when trying to compute in parallel'''
