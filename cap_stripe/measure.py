...
def get_corr_full_integral(sky_pix:pix_data, **kwargs):
    ...
    if kwargs['measure_flag'] in (const.CORR_FLAG, const.D_CORR2_FLAG):
        fullsky_corr = utils.parallel_correlation(sky_pix, **kwargs)
        full_int = np.sum(fullsky_corr ** 2)
    return full_int

#------------ Cap functions ------------
def get_cap_dcorr2(top:pix_data, bottom:pix_data, **kwargs):
    '''input_params: \n cap_angle, cutoff_ratio, nblocks, nsamples'''
    cap_angle, cutoff_ratio, nsamples = \
        kwargs['cap_angle'], kwargs['cutoff_ratio'], kwargs['nsamples']
    tctt = utils.parallel_correlation(top, **kwargs)
    bctt = utils.parallel_correlation(bottom, **kwargs)
...
def get_stripe_anomaly(sky_pix:pix_data, **kwargs):
    sampling_range, stripe_thickness, measure_flag, nsamples, nblocks = \
        kwargs['sampling_range'], kwargs['stripe_thickness'], kwargs['measure_flag'], kwargs['nsamples'], kwargs['nblocks']
    measure_results = np.zeros(len(sampling_range))
    f_int = get_corr_full_integral(sky_pix, nsamples, nblocks)
    ...
    for i in range(len(stripe_centers)):
        print("++ Stripe center {} degrees\r".format(stripe_centers[i]), end="")
